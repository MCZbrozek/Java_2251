...
{	//The JPanel that the game runs in
	...
	//Determine where the program time is being spent.
	...
	{	//Get computer screen size
		...
		System.out.println("Screen Width = "+Integer.toString(Main.ScreenWidth));
		System.out.println("Screen Height = "+Integer.toString(Main.ScreenHeight));
		System.out.println("Screen Ratio = "+Double.toString(((double)Main.ScreenWidth)/((double)Main.ScreenHeight)));
		System.out.println("Compare to 16/9 = "+Double.toString(16.0/9.0));
		initUI();
	}
	...
	{	/* This value would probably be stored elsewhere.
		...
		while (running)
		{
			...
			if (!paused)
			{
				...
				if(USE_PROFILER){ profiler.end(Profiler.COMPUTE_INDEX); }
				...
				if(USE_PROFILER){ profiler.start(); }
				drawGame(1.0f);
				if(USE_PROFILER){ profiler.end(Profiler.DRAW_INDEX); }
				lastRenderTime = now;
				...
				if(USE_PROFILER){ profiler.end(Profiler.YIELD_INDEX); }
			} //if (!paused)
		} //while (running)
	} //public void gameLoop()
	...
	{
		game_frame.internalUpdateGraphicsInterpolated(fps);
		frame_count++;
	}
