...
{	//Track whether or not the sprite is on the screen
	...
	public double getAngleToCoords(
	...
	{
		double goal_angle = getAngleToCoords(
			x_destination,
			y_destination);
		...
		double y_destination)
	{
		double goal_angle = getAngleToCoords(
				x_destination,
				y_destination);
		return Utils.getAngleDifference(this.angle, goal_angle);
	}
	...
	{
		return Utils.distance(getXCenter(), getYCenter(), destination_x, destination_y);
	}

	public double distanceTo(Sprite s)
	{
		return Utils.distance(getXCenter(), getYCenter(), s.getXCenter(), s.getYCenter());
	}
