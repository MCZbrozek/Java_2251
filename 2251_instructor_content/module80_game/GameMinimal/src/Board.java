...
public class Board extends JPanel 
{
	...
	//This method is used for when the camera
	//centers on something other than the
	...
	//Overloaded version for when you don't want to do testing
	public ArrayList<SpritePhysics> getLineCollisions(
		double origin_x, double origin_y,
		double angle, double length,
		double beam_width)
	{
		return getLineCollisions(
			origin_x, origin_y,
			angle, length,
			beam_width, false);
	}
	/* Returns list of sprites that 
	collide with the given line. Note 
	that the beam_width parameter
	currently has no effect on
	collisions with rectangles. */
	public ArrayList<SpritePhysics> getLineCollisions(
		double origin_x, double origin_y,
		double angle, double length,
		double beam_width, boolean testing)
	{
		...
		//Handle perfectly vertical beams
		if(dx == 0)
		{
			if(testing)
				System.out.println("WARNING in Board.getLineCollisions: Perfectly vertical beam.");
			return getLineCollisionsVertical(origin_x, origin_y, angle, length, beam_width, testing);
		}
		//Handle perfectly horizontal beams
		if(dy == 0)
		{
			if(testing)
				System.out.println("WARNING in Board.getLineCollisions: Perfectly horizontal beam. NOT YET IMPLEMENTED.");
			return to_return;
		}
		...
		for(int i=sprite_list.size()-1;i>=0;i--)
		{
			...
			{
				point_of_intersection = ((SpriteCircular)sp).beamIntersects(
					beam_width, y_intercept, slope, negative_reciprocal);
			}
			else //else must be a rectangular sprite
			{
				//Cast to SpriteRectangular
				SpriteRectangular sr = (SpriteRectangular)sp;
				//Get nearest intersection point
				point_of_intersection = sr.beamIntersects(slope, y_intercept, origin_x, origin_y);
			}
			...
			if(point_of_intersection != null)
			{
				...
				if(Utils.isBetween(origin_x, end_x, point_of_intersection.x) &&
				Utils.isBetween(origin_y, end_y, point_of_intersection.y))
				{
					to_return.add(sp);
					//TESTING START
					if(testing)
					{
						...
						DisplayLine l = new DisplayLine(origin_x,origin_y,0.1, end_x, end_y, (int)beam_width, Color.WHITE);
						this.addSpriteIntangible(l);
						//Draw perpendicular line
						double alt_angle = Math.atan2(-dx,dy);
						double alt_end_x = point_of_intersection.x+Math.cos(alt_angle)*length;
						double alt_end_y = point_of_intersection.y+Math.sin(alt_angle)*length;
						l = new DisplayLine(point_of_intersection.x,point_of_intersection.y,0.1, alt_end_x, alt_end_y, (int)beam_width, Color.WHITE);
						this.addSpriteIntangible(l);
						displayDotAtPoint(point_of_intersection);
					}
					//TESTING END
				}
			}
		...
		//TESTING
		System.out.println("number of collisions "+to_return.size());
		return to_return;
	}

	/* Returns list of sprites that collide with the given line, but it's a vertical line! */
	public ArrayList<SpritePhysics> getLineCollisionsVertical(
			double origin_x, double origin_y,
			double angle, double length,
			double beam_width, boolean testing)
	{
		System.out.println("TODO LEFT OFF HERE in GameMinimal's Board.java This method is not complete.");
		//Store the sprites that hit the line here.
		ArrayList<SpritePhysics> to_return = new ArrayList<>();
		//Calculate the change in x and change in y.
		double dx = Math.cos(angle);
		double dy = Math.sin(angle);
		//Calculate slope and other useful variables
		double end_x = origin_x;
		double end_y = origin_y+Math.sin(angle)*length;
		double slope = Double.NaN;
		double negative_reciprocal = 0;
		double y_intercept = Double.NaN;
		Point point_of_intersection;
		//Loop over all the sprites
		//for (SpritePhysics sp : sprite_list)
		for(int i=sprite_list.size()-1;i>=0;i--)
		{
			...
			if(sp instanceof SpriteCircular)
			{
				point_of_intersection = ((SpriteCircular)sp).vertBeamIntersects(beam_width,origin_x);
			}
			else //else must be a rectangular sprite
			{
				//Get nearest intersection point
				//TODO
				//point_of_intersection = ((SpriteRectangular)sp).beamIntersects(slope, y_intercept, origin_x, origin_y);
			}
			//Since the sprite collided. Check if the sprite is within
			// the length of the line.
			if(point_of_intersection != null)
	...
	private void displayDotAtPoint(Point p)
	{
		DisplayDot d = new DisplayDot(
			p.x, p.y,
			0.1, //duration
			5, //radius
			Color.RED);
		this.addSpriteIntangible(d);
	}
	...
	/** Private class used for keeping track of mouse inputs. */
