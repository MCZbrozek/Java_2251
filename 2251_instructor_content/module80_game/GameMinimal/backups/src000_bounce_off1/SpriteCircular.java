...
public abstract class SpriteCircular extends SpritePhysics
{
	...
	public void bounceOff(Sprite other)
	{
		...
		//the wall.
		this.setVelocity(
				w_vector[0]*this.getBounceFriction()-u_vector[0]*this.getElasticity(),
				w_vector[1]*this.getBounceFriction()-u_vector[1]*this.getElasticity());
		...
		//Check if we were hit from behind
		if(Math.abs(angle_difference) > Math.PI*3.0/4.0)
		{
			//Boost forward a bit based on the normal vector
			this.changeVelocity(
					normal_vector[0],
					normal_vector[1]);
		}
