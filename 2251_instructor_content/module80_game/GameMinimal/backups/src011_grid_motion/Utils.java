...
import java.io.File;
import java.util.Random; //TODO Should use SecureRandom instead.
...
public class Utils 
{
			...
			double negative_reciprocal)
	{
		...
		//of intersection to the other sprite to
	...
	/** Check to see if the beam runs through the
	sprite using rectangular collisions.
	Inputs: sprite s and beam slope and y_intercept,
	and the beam origin.
	Outputs: Point of intersection nearest to the
	beam origin.*/
			...
			double beam_x_origin, double beam_y_origin)
	{
		...
		if(s.getX() < intercept.x &&
		intercept.x < s.getX()+s.getWidth())
		{
			temp_dist = Utils.distance(
					beam_x_origin, beam_y_origin,
					intercept.x, intercept.y);
		...
		intercept = Utils.getInterceptVertical(
				slope, y_intercept,
				s.getX());
		if(s.getY() < intercept.y &&
		intercept.y < s.getY()+s.getHeight())
		{
			temp_dist = Utils.distance(
					beam_x_origin, beam_y_origin,
					intercept.x, intercept.y);
