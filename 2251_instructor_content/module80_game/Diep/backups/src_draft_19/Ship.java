...
RAM
(Control-f search this file for SHAPE_ for more information)
upgrades from initial ship
speed UP x1.3
does not shoot
...
cloaks and decloaks from NPCs for intervals between 0 and 5 seconds at random. Otherwise NPCs woud never be fooled.
...
public class Ship extends SpriteTopDown
{
    ...
    public void update(double elapsed_seconds)
    {
        ...
        else if(this.shape == Constants.SHAPE_CREEPER)
        {
    ...
    int getXpReward(){ return 40+this.level*4; }
    ...
    * the invicibility powerup. */
    ...
    boolean cloaked(){ return (this.shape==Constants.SHAPE_MIMIC || this.shape==Constants.SHAPE_CREEPER) && cloak_cooldown<0; }
    ...
    public void draw(Board b, Graphics g, int offset_x, int offset_y)
    {
        ...
        if(this.isVisible() && !this.powerup_on[Constants.POW_MIMIC] && this.shape!=Constants.SHAPE_MIMIC)
        {
            ...
            {   //Draw this enemy's name
                ...
                //Draw cloak cooldown
                if(this.shape==Constants.SHAPE_CREEPER)
                {
