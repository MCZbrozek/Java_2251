...
public class Ship extends SpriteTopDown
{
    ...
    {
        //Rammer ship does not shoot
        if(this.shape == Constants.SHAPE_RAMMER){return;}
        //Standard bullet radius. This is modified below.
        int bullet_radius = 6;
        //Change some bullet attributes if the BIG_BULLETS powerup is active
        int modifier = 1;
        if(cooldowns[Constants.POW_BIG_BULLETS]>0){ modifier = 8; }
        ...
        if(cooldowns[Constants.POW_SPREAD_SHOT]>0)
        {
            temp_shot = new Shot(
                    ...
                    bullet_hits_bitmask,
                    this,
                    angle+Math.PI/6.0+(Math.random()-0.5)*bullet_spread,
                    bullet_longevity*modifier,
                    bullet_damage,
                    bullet_speed,
                    bullet_health*modifier,
                    bullet_radius*modifier,
    ...
    void attack()
    {
        if(firing_cooldown<=0)
        {
            ...
            //Check if this is the flanker ship.
            if(this.shape == Constants.SHAPE_FLANKER)
            {   //Also fire backwards if this is a flanker
                this.shoot(this.getAngle()+Math.PI);
            }
        ...
        else if(cooldowns[Constants.POW_TRIPLE_SHOT] > 0 && triple_shot_cooldown<0 && triple_shot_count > 0)
        {
            triple_shot_count--;
            triple_shot_cooldown = 0.1;
            this.shoot(this.getAngle());
