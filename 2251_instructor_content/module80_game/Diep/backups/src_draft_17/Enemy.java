...
public class Enemy extends Ship
{
    ...
    {   //Check if it's time to get new targets
        ...
        {
            this.shootAttack(elapsed_seconds, nearest_ship, "ship");
        }
        else
        {
    ...
    private void ramAttack(double elapsed_seconds, SpritePhysics target, String extra_info)
    {   //Orient gun away from the target for extra recoil thrust!
        this.setAngle(target.angleToSprite(this));
        this.moveToward(elapsed_seconds, target.getXCenter(), target.getYCenter());
        /*strategy = "ram target "+extra_info;
    ...
    private void shootAttack(double elapsed_seconds, SpritePhysics target, String extra_info)
    {   //Orient gun toward the target
        this.setAngle(this.angleToSprite(target));
        this.moveToward(elapsed_seconds, target.getXCenter(), target.getYCenter());
        /*strategy = "shoot target "+extra_info;
        board_reference.addSpriteIntangible(
                new DisplayLine(this.getXCenter(), this.getYCenter(),
                        0.05,
                        target.getXCenter(),
    ...
    private void fleeShoot(double elapsed_seconds, SpritePhysics target, String extra_info)
    {   //Orient gun toward the target
        this.setAngle(this.angleToSprite(target));
        this.moveAwayFrom(elapsed_seconds, target.getXCenter(), target.getYCenter());
        /*strategy = "flee shoot "+extra_info;
        board_reference.addSpriteIntangible(
                new DisplayLine(this.getXCenter(), this.getYCenter(),
                        0.05,
                        target.getXCenter(),
    ...
    private void shooterBehavior(double elapsed_seconds)
    {
        ...
        * Otherwise attack the nearest experience brick. */
        if(this.distanceTo(nearest_ship)<500 || this.getLevel()>19)
        {
            ...
            {   //Maintain distance
                if(this.distanceTo(nearest_ship)<300)
                {
                    this.fleeShoot(elapsed_seconds, nearest_ship, "ship");
                }
                else
                {   //Attack!
                    this.shootAttack(elapsed_seconds, nearest_ship, "ship");
        ...
        {   //Ram the powerup
            this.ramAttack(elapsed_seconds, nearest_powerup, "powerup");
        }
        else
        {   //Otherwise attack the nearest experience brick.
            //Maintain safe distance from target
            if(this.distanceTo(nearest_xp)<120)
            {
                this.fleeShoot(elapsed_seconds, nearest_xp, "XP "+nearest_xp.getHealth());
            }
            else
            {
                this.shootAttack(elapsed_seconds, nearest_xp, "XP "+nearest_xp.getHealth());
            }
        }
    }
    ...
    private void rammerBehavior(double elapsed_seconds)
    {
        ...
        * Be more likely to farm at lower levels. */
        ...
        {   //Otherwise ram the nearest experience brick.
            this.ramAttack(elapsed_seconds, nearest_xp, "XP "+nearest_xp.getHealth());
        }
    }
