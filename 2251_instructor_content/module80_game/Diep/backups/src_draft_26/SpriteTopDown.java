...
public abstract class SpriteTopDown extends SpriteCircular
{
    ...
    double friction = Constants.default_friction;
    ...
    //Push the sprite in bounds
    private void pushInBounds(double elapsed_seconds)
    {
        if (this.getX() < 0)
        {
            this.changeVelocity(Constants.inbounds_impulse * elapsed_seconds, 0);
        }
        else if (this.getX() + this.getCollisionRadius() * 2 > Constants.boundary_right)
        {
            this.changeVelocity(-Constants.inbounds_impulse * elapsed_seconds, 0);
        }
        if (this.getY() < 0)
        ...
        else if (this.getY() + this.getCollisionRadius() * 2 > Constants.boundary_down)
        {
            this.changeVelocity(0, -Constants.inbounds_impulse * elapsed_seconds);
        }
    }

    //The walls are hard boundaries. Bounce the sprite in bounds
    private void bounceInBounds()
    {
        ...
        {
            this.setX(Constants.boundary_right - this.getCollisionRadius()*2);
            this.setVelocity(-this.getdx(), this.getdy());
        }
        if (this.getY() < 0)
        {
        ...
        {
            this.setY(Constants.boundary_down - this.getCollisionRadius()*2);
            this.setVelocity(this.getdx(), -this.getdy());
        }
    }
        ...
        for (SpritePhysics temp_sprite : nearby_sprites)
        {
                    ...
                    ))
            {
                ...
                nearest_xp = (ExperienceBrick) temp_sprite;
                    ...
                    temp_sprite instanceof Shot &&
                    ((Shot)temp_sprite).getShooter()!=this
                    && ((Shot)temp_sprite).getShooter().getTeam()!=((Ship)this).getTeam()
                    && this.distanceTo(temp_sprite) < closest_distance_shot)
            {
                ...
                nearest_shot = (Shot) temp_sprite;
                    ...
                    && !(this instanceof Ship && ((Ship)temp_sprite).getTeam()!=((Ship)this).getTeam())
    ...
    {   //If other is this sprite's own bullet, just return
        ...
        {   //Damage the other unless self and other are both experience
            ...
            //other.
            //Also, invincible experience bricks no longer damage ships
            if(!(this instanceof ExperienceBrick && other instanceof ExperienceBrick)
                    && !(other instanceof Ship && this instanceof ExperienceBrick && ((ExperienceBrick) this).invincible))
            {
