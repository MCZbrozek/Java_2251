...
public class NPC_Smuggley extends NPC_Personality
{
    private Random rng = new Random();

    public NPC_Smuggley(Board b)
    {
        super(b);
        ...
        String[] temp_upgrade_sequence = {"Bullet Damage", "Penetration", "Bullet Damage", "Penetration", "Health Regen", "Refire Rate", "Bullet Speed", "Bullet Longevity", "Refire Rate", "Bullet Longevity", "Refire Rate", "Refire Rate", "Bullet Speed", "Refire Rate", "Bullet Speed", "Bullet Longevity", "Penetration", "Health", "Bullet Damage", "Bullet Speed", "Bullet Longevity", "Bullet Speed", "Bullet Longevity", "Penetration", "Health", "Health Regen", "Refire Rate", "Refire Rate", "Bullet Speed", "Bullet Longevity", "Penetration", "Bullet Damage"};
    ...
    void update(double elapsed_seconds, Ship s)
    {
        if (s.getLevel() >= 25)
        {
            ...
            if(s.nearest_shot!=null && s.distanceTo(s.nearest_shot)<250)
            {
        ...
        else if(s.nearest_ship!=null && s.distanceTo(s.nearest_ship)<200)
        {
            //If it is weak or my level is high, kill it!
            if(s.nearest_ship.getHealth() < s.getHealth() || s.getLevel() > (s.nearest_ship.getLevel() + 5))
            {
                this.shootAttack(elapsed_seconds, s.nearest_ship, s, "I smell blood in the water!");
            }
            else
        ...
        else if(s.nearest_shot!=null && s.distanceTo(s.nearest_shot)<500)
        {
            this.fleeShoot(elapsed_seconds, s.nearest_ship, s, "I can dodge bullets.");
        }
