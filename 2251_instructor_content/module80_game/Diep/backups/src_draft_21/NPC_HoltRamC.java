...
public class NPC_HoltRamC extends NPC_Personality
{
    private boolean recovery_state = false;
    private ExperienceBrick memory_brick;

    public NPC_HoltRamC(Board b)
    {
        super(b);
        this.debugging=true;
        /* rammer strat
    ...
    void shootMove(double elapsed_seconds, SpritePhysics target1, SpritePhysics target2, Ship s)
    ...
    ExperienceBrick getNearestBigXp(Ship s)
    {
    ...
    void gunBehavior(double elapsed_seconds, Ship s)
    {
        ...
        //Get nearest big xp brick
        memory_brick = this.getNearestBigXp(s);
        ...
        {
            this.shootMove(elapsed_seconds, s.nearest_ship, memory_brick, s);
        }
        //Opportunistically attack very weak ships nearby
        else if(s.nearest_ship!=null && s.nearest_ship.getHealth()<30 && s.distanceTo(s.nearest_ship)<500)
        {
        ...
        else
        {
            ...
            {   //Shoot nearest brick. Move toward nearest big xp brick
                this.shootMove(elapsed_seconds, s.nearest_xp, memory_brick, s);
            }
        }
        //Shoot as often as possible.
        s.attack();
    ...
    void ramBehavior(double elapsed_seconds, Ship s)
    {
        /*
        rammer strat
        -aggressively seek powerups
        ...
        -upgrade all regen, 1 health,
         ...
         2 health, then max damage, put some more in speed and health.
        -wait near center for ambush*/
        //TODO Get nearest high value brick and killable enemy
        ArrayList<SpritePhysics> list = s.board_reference.getSpriteList();
        SpriteTopDown nearest_big_xp = null;
        double dist = Double.MAX_VALUE;
        ...
        for (SpritePhysics sp : list)
        {
            if(sp instanceof ExperienceBrick &&
                    ((ExperienceBrick)sp).getXpReward()==11 &&
                    s.distanceTo((SpriteTopDown)sp)<dist)
            {
                nearest_big_xp = (SpriteTopDown)sp;
                dist = s.distanceTo((SpriteTopDown)sp);
            }
            //Other is a ship, not me, and killable in 3 or fewer hits
            else if(sp instanceof Ship && sp != s &&
                ((Ship)sp).getHealth()/s.body_damage <= 3.0 &&
                s.distanceTo((SpriteTopDown)sp)<dist_enemy)
            {
                nearest_weak_enemy = (SpriteTopDown)sp;
                dist_enemy = s.distanceTo((SpriteTopDown)sp);
            }
        }
        //TODO if low health: enter recovery state.
        if(s.getHealth()<30)
        {
            recovery_state = true;
        }
        //TODO if health is above 80%: exit recovery state
        else if(s.getPercentHealth()>=0.8)
        {
            recovery_state = false;
        }
        //TODO if recovery state:
        ...
        //TODO elif my level > 27: wait for ambush
        else if(s.getLevel()>27)
        {
            if(s.nearest_ship.getHealth() < s.getHealth() || s.distanceTo(s.nearest_ship)<350)
            {   //Attack
        ...
        //TODO: nearest enemy is near and I have lots of health. attack them
        else if(s.distanceTo(s.nearest_ship)<500 && s.getPercentHealth()>0.6)
        {
            this.ramAttack(elapsed_seconds, s.nearest_ship, s, "Attack ship!");
        }
        //TODO else: find nearest powerup, highest value brick, enemy I can kill in 5 or fewer hits
	    //seek whichever is closest
        else
        {   //Get nearest of big xp brick, powerup, killable enemy.
            SpriteTopDown nearest = nearest_big_xp;
            String text = "Get XP";
            if(nearest_weak_enemy!=null && s.distanceTo(nearest_weak_enemy) < s.distanceTo(nearest))
            {
                nearest = nearest_weak_enemy;
                text = "Blood in the water!";
            }
            if(s.nearest_powerup!=null && s.distanceTo(s.nearest_powerup) < s.distanceTo(nearest))
            {
    ...
    void died(Ship s)
    {
        ...
        if(s.getLives()<2 || s.getLevel()>20)
        {
            ...
            int[] temp_ship_upgrade_sequence = {
                    Constants.SHAPE_RAMMER,
