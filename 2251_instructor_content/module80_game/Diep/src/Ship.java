...
RICOCHET RAM
(Control-f search this file for SHAPE_RICOCHET for more information)
Upgrades from rammer.
No friction.
Bounces off all objects.
Does lots of body damage (sum of bullet and collision damage times 2.

RICOCHET SHOT
(Control-f search this file for SHAPE_RICOCHET_SHOT for more information)
Upgrades from machine gun.
Fires ricochet bullets.
Damage is increased by each bounce.
Penetration determines number of bounces.
*/
...
public class Ship extends SpriteTopDown
{
	...
	void setAttributes()
	{
		...
		//In billiards mode, upgrading collision damage increases
		//shove out power.
		if(BoardTopDown.game_mode==BoardTopDown.BILLIARDS_MODE)
		{
			this.shove_out_impulse = this.shove_out_impulse*(collision_damage_level+1);
		}
		...
		else if(this.shape == Constants.SHAPE_RICOCHET)
		{
			this.friction = -0.1; //Small friction
			//Does lots of body damage (sum of bullet and collision damage times 2.
			this.setBodyDamage((Constants.collision_damage_levels[collision_damage_level]+this.bullet_damage)*2);
			//larger ship
	...
	void setAttributes(double speed, 
		int bullet_damage, double bullet_speed,
		double refire_rate, double bullet_longevity,
		int health, double regen_cooldown_reset,
		int body_damage, int bullet_health)
	{
		this.speed = speed;
	...
	private void createBullet(double angle, double x, double y)
	{
		...
		else if(this.shape == Constants.SHAPE_LASER)
		{
			...
			{   //Don't hit self or powerups or teammates
				if(sp!=this && !(sp instanceof PowerUp) && !(sp instanceof Ship && ((Ship) sp).getTeam()==this.team_id))
				{
		...
		else
		{
			temp_shot = new Shot(x,y,
					...
					this.powerup_on[Constants.POW_EXPLODING_BULLETS] ||
							Constants.bullet_explodes_default ||
							this.shape==Constants.SHAPE_EXPLODER,
					this.shape == Constants.SHAPE_RICOCHET_SHOT,
					board_reference);
		}
	...
	{   //Rammer, cloaker, and gravity ships do not shoot
		if(this.shape == Constants.SHAPE_RAMMER ||
				...
				this.shape == Constants.SHAPE_MIMIC ||
				this.shape == Constants.SHAPE_CLOAK ||
				this.shape == Constants.SHAPE_RICOCHET)
		{
			return;
		}
	...
	/** Intermediary function to see if this 
	 * is a special ship shape with a 
	 * special firing mechanism. */
	...
	void attack()
	{
		...
		else if(this.powerup_on[Constants.POW_TRIPLE_SHOT] && triple_shot_cooldown<0 && triple_shot_count > 0)
		{
			triple_shot_count--;
			triple_shot_cooldown = 0.1;
			this.shootIntermediary(); //System.out.println("shoot2 tripleshot"); //TESTING
		}
	}
	...
	void levelUpAttribute(String name)
	{
		switch (name)
		{
			...
			case "Penetration":
				...
				break;
			default:
				System.out.println("Error in Ship.levelUpAttribute('"+name+"')");
				System.exit(0);
		}
		this.setAttributes();
	}
	...
	int getAttributeLevel(String name)
	{
		switch (name)
		{
			...
			default:
				System.out.println("Error in Ship.getAttributeLevel('"+name+"')");
				System.exit(0);
		}
		return 0;
	}
	...
	{
		if(BoardTopDown.game_mode==BoardTopDown.BILLIARDS_MODE)
		{   //No points for killing in billiards mode
			return 0;
		}
		else
		{
			return 40+this.getLevel()*4;
		}
	}
	...
	/* Override parent class's takeDamage so we can check
	* the invicibility powerup and friendly fire. */
	public void takeDamage(int damage, Sprite damage_dealer)
	{
		//Don't take damage from teammates. No friendly fire.
		if(damage_dealer instanceof Ship && ((Ship) damage_dealer).getTeam() == this.team_id)
		{
			return;
		}
		else if(damage_dealer instanceof Shot && ((Shot) damage_dealer).getShooter().getTeam() == this.team_id)
		{
			return;
		}
		else if(damage_dealer instanceof Seeker && ((Seeker) damage_dealer).getShooter().getTeam() == this.team_id)
		{
			return;
		}
		...
		if(this.getHealth()<=0)
		{
			//Decrease number of lives in FFA_mode
			if(BoardTopDown.game_mode==BoardTopDown.FFA_MODE)
			{
				this.lives -= 1;
				//20% xp reduction
				this.reduceXP(0.20);
			}
			//Reset if more lives remain
			if(this.lives>=1)
			{
			...
			{	//Otherwise it's a player
				//and we should tell the board
				//to recenter on this sprite.
				//If you don't do this, then
				//sniper mode look_ahead will
				//persist after death. One
				//downside of this is that in
				//spectator mode if the player
				//dies off screen, focus may
				//jump back to the player.
				board_reference.centerOnSprite(this, 0);
			}
		}
	}
	...
	int getBulletDamage(){return bullet_damage;}
	...
	public void handleCollision(SpritePhysics other)
	{
		...
		//Ricochet ship bounces off others except bullets
		if(this.shape == Constants.SHAPE_RICOCHET && !(other instanceof Shot) && !(other instanceof Seeker))
		{
			this.bounceOff(other);
		}
	}

	int getTeam(){ return this.team_id; }
	
	public boolean isSniperShaped()
	{
		return this.shape == Constants.SHAPE_SNIPER ||
		this.shape == Constants.SHAPE_CANNON ||
