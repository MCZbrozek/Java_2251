...
class LevelUpInterface
{
    ...
    private final int x = 40; //Starting x for plus signs
	private final int x_text = 65; //Starting x for text
    private final int y = 100;
	private final int rect_width = 10;
	private final int rect_width_buffer = 2;
    private final int width = rect_width+rect_width_buffer;
    ...
    void draw(Graphics g)
    {
        ...
        if(levelup_points>0 || display)
        {
            ...
            for (int i = 0; i <Constants.attribute_names.length; i++)
            {
                ...
                {
                    g.fillRect(x_text+j*(rect_width+rect_width_buffer),y+i*(height+buffer), rect_width, height);
                }
                g.drawString(name,
					x_text+level*width,
					y+i*(height+buffer)+extra_text_buffer);
